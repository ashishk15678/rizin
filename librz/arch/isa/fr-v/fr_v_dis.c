...
//
// Fujitsu FR-V (RISC-VLIW) disassembler core — frv_dis.c
//
// Instruction encoding reference:
//   • GCC gcc/config/frv/frv.c  (Red Hat/FSF, GPL-3)
//     https://chromium.googlesource.com/native_client/pnacl-gcc/+/cf3f40121df7fd852a0942d12170bdaea3c22651/gcc/config/frv/frv.c
//   • GNU Binutils opcodes/frv-opc.c, frv-desc.c  (CGEN auto-generated, GPL-3)
//   • Fujitsu FR Family Instruction Manual CM71-00101-5E
//   • FR-V ISA overview on HandWiki
//
// Code structure modelled after rizin/librz/asm/arch/cil/cil_dis.c
// and rizin/librz/asm/arch/hexagon/hexagon_arch.c (both LGPL-3.0)
...
// Integer immediate arithmetic sub-opcodes (major 0x00, GRJ field used as simm12)
// Differentiated from register form by bit 11 of the raw word being set in some
// encodings; here we detect by looking at a dedicated "imm-form" sub-major bit.
...
// Logical sub-opcodes (major 0x01)
...
// Integer compare sub-opcodes (major 0x03)
static const char *const frv_cmp_mnem[4] = {
    [0] = "cmp",    // set ICC, no dest
    [1] = "cmpi",   // compare with immediate
    [2] = "andcc",  // and setting CC
    [3] = "orcc",   // or setting CC
};
...
static const struct { const char *ld; const char *st; } frv_fls_ops[8] = {
...
static void decode_int_logic(FRVOp *op, ut32 insn, ut64 pc) {
    ...
    switch (subop) {
    case 0: case 1: case 2: case 3: // reg forms
        rz_strbuf_appendf(&op->strbuf, "%s %s, %s, %s", mn, gr(gri), gr(grj), gr(grk));
        break;
    case 4: case 5: case 6: // immediate forms
...
static void decode_int_shift(FRVOp *op, ut32 insn, ut64 pc) {
    ...
    switch (subop) {
    case 0: case 1: case 2: // register shift amount
        rz_strbuf_appendf(&op->strbuf, "%s %s, %s, %s", mn, gr(gri), gr(grj), gr(grk));
        break;
    case 3: case 4: case 5: case 6: // immediate shift amount
...
// ── Set/move group (major 0x04) ───────────────────────────────────────────────
//
// Encoding (from Fujitsu FR Family Instruction Manual and GCC frv.md):
//   sethi:  op=0x04, subop=0, GRj=src (ignored), 16-bit hi → GRk
//   setlo:  op=0x04, subop=1
//   setlos: op=0x04, subop=2 (combined set, signed)
//   movgf:  op=0x04, subop=4 — move GR → FR
//   movfg:  op=0x04, subop=5 — move FR → GR
//   movgfd: op=0x04, subop=6 — move GR double → FR double
//   movfgd: op=0x04, subop=7
...
// The CCi field [11:6] encodes both the condition register (CR0-CR7) and
// the sense (true/false) of the condition.
