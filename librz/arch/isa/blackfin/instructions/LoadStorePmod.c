...
bool DisassembleLoadStorePmod(uint16_t instructionWord, struct Instruction *instr, bool parallel) {
	/* LDSTpmod
	+---+---+---+---|---+---+---+---|---+---+---+---|---+---+---+---+
	...
	if (aop == 1 && W == 0 && idx == ptr) {
		...
		instr->operands[0] = { .cls = REG, .reg = dregs_lo(reg) };
		instr->operands[1] = { .cls = OPERATOR, .operat = OPER_EQ };
		instr->operands[2].cls = MEM_ACCESS;
		instr->operands[2].mem_access = { .mode = MEM_REG, .ptr_reg = pregs(ptr), .width = 2 };
	} else if (aop == 2 && W == 0 && idx == ptr) {
		// reg_hi = W[ptr]
		instr->operand_count = 3;
		instr->operation = OP_LD;
		instr->operands[0] = { .cls = REG, .reg = dregs_hi(reg) };
		instr->operands[1] = { .cls = OPERATOR, .operat = OPER_EQ };
		instr->operands[2].cls = MEM_ACCESS;
		instr->operands[2].mem_access = { .mode = MEM_REG, .ptr_reg = pregs(ptr), .width = 2 };
		...
		instr->operands[0].cls = MEM_ACCESS;
		instr->operands[0].mem_access = { .mode = MEM_REG, .ptr_reg = pregs(ptr), .width = 2 };
		instr->operands[1] = { .cls = OPERATOR, .operat = OPER_EQ };
		instr->operands[2] = { .cls = REG, .reg = dregs_lo(reg) };
		...
		instr->operands[0].mem_access = { .mode = MEM_REG, .ptr_reg = pregs(ptr), .width = 2 };
		instr->operands[1] = { .cls = OPERATOR, .operat = OPER_EQ };
		instr->operands[2] = { .cls = REG, .reg = dregs_hi(reg) };
	} else if (aop == 0 && W == 0) {
		// reg = [ptr ++ idx]
