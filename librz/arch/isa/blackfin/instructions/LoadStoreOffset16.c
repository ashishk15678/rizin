...
bool DisassembleLoadStoreOffset16(uint16_t instructionWordHigh, uint16_t instructionWordLow, struct Instruction *instr, bool parallel) {
	/* LDSTidxI
	+---+---+---+---|---+---+---+---|---+---+---+---|---+---+---+---+
	...
	if (W == 0 && sz == 0 && Z == 0) {
		...
		instr->operands[2] = { .cls = MEM_ACCESS, .mem_access = { .mode = MEM_REGOFF, .ptr_reg = pregs(ptr), .offset = imm16s4(offset), .oper = OPER_PLUS, .width = 4 } };
		...
		instr->operands[0] = { .cls = REG, .reg = dregs(reg) };
		instr->operands[2] = { .cls = MEM_ACCESS, .mem_access = { .mode = MEM_REGOFF, .ptr_reg = pregs(ptr), .offset = imm16s2(offset), .oper = OPER_PLUS, .width = 2 } };
		...
		instr->operands[0] = { .cls = REG, .reg = dregs(reg) };
		instr->operands[2] = { .cls = MEM_ACCESS, .mem_access = { .mode = MEM_REGOFF, .ptr_reg = pregs(ptr), .offset = imm16(offset), .oper = OPER_PLUS, .width = 1 } };
		instr->operands[2].flags.zero_extended = true;
		...
		// [preg + offset] = dreg
		instr->operands[0] = { .cls = MEM_ACCESS, .mem_access = { .mode = MEM_REGOFF, .ptr_reg = pregs(ptr), .offset = imm16s4(offset), .oper = OPER_PLUS, .width = 4 } };
		...
		// W[preg + offset] = dreg
		instr->operands[0] = { .cls = MEM_ACCESS, .mem_access = { .mode = MEM_REGOFF, .ptr_reg = pregs(ptr), .offset = imm16s2(offset), .oper = OPER_PLUS, .width = 2 } };
		instr->operands[2] = { .cls = REG, .reg = dregs(reg) };
	} else if (W == 1 && sz == 2 && Z == 0) {
		// B[preg + offset] = dreg
		instr->operands[0] = { .cls = MEM_ACCESS, .mem_access = { .mode = MEM_REGOFF, .ptr_reg = pregs(ptr), .offset = imm16(offset), .oper = OPER_PLUS, .width = 1 } };
		instr->operands[2] = { .cls = REG, .reg = dregs(reg) };
	} else
		return false;
