...
bool DisassembleLoadStoreOffset(uint16_t instructionWord, struct Instruction *instr, bool parallel) {
	/* LDSTii
	+---+---+---+---|---+---+---+---|---+---+---+---|---+---+---+---+
	...
	if (W == 0 && op == 0) {
		...
		instr->operands[2] = { .cls = MEM_ACCESS, .mem_access{ .mode = MEM_REGOFF, .ptr_reg = pregs(ptr), .offset = uimm4s4(offset), .oper = OPER_PLUS, .width = 4 } };
	} else if (W == 0 && op == 1) {
		// dreg = W[preg + offset] (Z)
		instr->operands[0] = { .cls = REG, .reg = dregs(reg) };
		instr->operands[2] = { .cls = MEM_ACCESS, .mem_access{ .mode = MEM_REGOFF, .ptr_reg = pregs(ptr), .offset = uimm4s2(offset), .oper = OPER_PLUS, .width = 2 } };
		...
		// [preg + offset] = dreg
		instr->operands[0] = { .cls = MEM_ACCESS, .mem_access{ .mode = MEM_REGOFF, .ptr_reg = pregs(ptr), .offset = uimm4s4(offset), .oper = OPER_PLUS, .width = 4 } };
		instr->operands[2] = { .cls = REG, .reg = dregs(reg) };
	} else if (W == 1 && op == 1) {
		// W[preg + offset] = dreg
		instr->operands[0] = { .cls = MEM_ACCESS, .mem_access{ .mode = MEM_REGOFF, .ptr_reg = pregs(ptr), .offset = uimm4s2(offset), .oper = OPER_PLUS, .width = 2 } };
		instr->operands[2] = { .cls = REG, .reg = dregs(reg) };
	} else if (W == 1 && op == 3) {
		// [preg + offset] = preg
		instr->operands[0] = { .cls = MEM_ACCESS, .mem_access{ .mode = MEM_REGOFF, .ptr_reg = pregs(ptr), .offset = uimm4s4(offset), .oper = OPER_PLUS, .width = 4 } };
