...
bool DisassemblePushPopMultiple(uint16_t instructionWord, struct Instruction *instr, bool parallel) {
	...
	InstructionOperand stack_op;
	InstructionOperand range;
	...
	if (W) { // PUSH
		stack_op = { .cls = MEM_ACCESS, .mem_access = { .mode = MEM_REGMOD, .ptr_reg = REG_SP, .pre_dec = true } };
		instr->operands[0] = stack_op;
		instr->operands[2] = range;
		instr->operation = OP_PUSHMULT;
		return true;
	} else { // POP
		stack_op = { .cls = MEM_ACCESS, .mem_access = { .mode = MEM_REGMOD, .ptr_reg = REG_SP, .post_inc = true } };
		instr->operands[0] = range;
		instr->operands[2] = stack_op;
		instr->operation = OP_POPMULT;
		return true;
	}
}
