...
bool DisassembleLoadStoreFPOffset(uint16_t instructionWord, struct Instruction *instr, bool parallel) {
	/* LDSTiiFP
	+---+---+---+---|---+---+---+---|---+---+---+---|---+---+---+---+
	...
	if (W == 0) {
		...
		instr->operands[2] = { .cls = MEM_ACCESS, .mem_access = { .mode = MEM_REGOFF, .ptr_reg = REG_FP, .offset = negimm5s4(offset), .oper = OPER_PLUS, .width = 4 } };
	} else if (W == 1) {
		// [FP - offset] = reg
		instr->operation = OP_ST;
		instr->operands[0] = { .cls = MEM_ACCESS, .mem_access = { .mode = MEM_REGOFF, .ptr_reg = REG_FP, .offset = negimm5s4(offset), .oper = OPER_PLUS, .width = 4 } };
