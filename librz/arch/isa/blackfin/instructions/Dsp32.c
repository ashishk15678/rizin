#include "../include/Dsp32.h"

void decode_optmode(int mod, int MM, struct Instruction *instr) {
	if (mod == 0 && MM == 0)
		return;
...
int decode_multfunc(int h0, int h1, int src0, int src1, struct Instruction *instr) {
	enum Register s0, s1;
...
int decode_macfunc(int which, int op, int h0, int h1, int src0, int src1, struct Instruction *instr) {
	// const char *a;
...
void amod0(int s0, int x0, struct Instruction *instr) {
	if (s0 == 1 && x0 == 0) {
...
void amod1(int s0, int x0, struct Instruction *instr) {
	if (s0 == 0 && x0 == 0) {
...
void amod0amod2(int s0, int x0, int aop0, struct Instruction *instr) {
	if (s0 == 1 && x0 == 0 && aop0 == 0) {
		instr->operands[instr->operand_count++] = { .cls = MNEMOMIC, .mnemonic = OL_SMOD };
		instr->flags.S = true;
		// OUTS (outf, " (S)");
...
void searchmod(int r0, struct Instruction *instr) {
	if (r0 == 0) {
...
void aligndir(int r0, struct Instruction *instr) {
	if (r0 == 1) {
