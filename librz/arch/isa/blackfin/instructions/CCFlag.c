...
bool DisassembleCCFlag(uint16_t instructionWord, struct Instruction *instr, bool parallel) {
	...
	instr->operation = OP_CCFLAG;
	instr->operand_count = 5;

	if (opc == 5 || opc == 6 || opc == 7) {
		instr->operands[2].cls = REG;
		instr->operands[2].reg = REG_A0;
		instr->operands[4].cls = REG;
		instr->operands[4].reg = REG_A1;
		switch (opc) {
		case 5:
			instr->operands[3].operat = OPER_EQEQ;
			return true;
		case 6:
			instr->operands[3].operat = OPER_LT;
			return true;
		case 7:
			instr->operands[3].operat = OPER_LTE;
			return true;
		}
	}
	...
	case 2:
		...
		break;
	case 3:
		instr->operands[3].operat = OPER_LTU;
		break;
	case 4:
		instr->operands[3].operat = OPER_LTEU;
		break;
	}
	return true;
}
