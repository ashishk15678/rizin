...
bool DisassembleBranchCC(uint16_t instructionWord, struct Instruction *instr, bool parallel) {
	...
	instr->operation = OP_BRCC;
	instr->operands[0].cls = MNEMOMIC;
	instr->operands[0].mnemonic = OL_IF;
	instr->operands[1].cls = REG;
	instr->operands[1].reg = REG_CC;
	instr->operands[2].cls = MNEMOMIC;
	instr->operands[2].mnemonic = OL_JUMP;
	instr->operands[3].cls = IMM;
	instr->operands[3].imm = pcrel10(offset);
	instr->operands[3].flags.pcrelative = true;

	if (B)
		instr->operands[3].flags.branch_predicted = true;
	if (!T)
		instr->operands[1].flags.cc_inverted = true;
	return true;
}
