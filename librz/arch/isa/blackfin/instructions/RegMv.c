...
bool DisassembleRegMv(uint16_t instructionWord, struct Instruction *instr, bool parallel) {
	...
	instr->operands[0] = { .cls = REG, .reg = allregs(dst, gd) };
	instr->operands[1] = { .cls = OPERATOR, .operat = OPER_EQ };
	instr->operands[2] = { .cls = REG, .reg = allregs(src, gs) };
